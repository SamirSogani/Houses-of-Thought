...
- **Within a layer:** the `n` perspective bundles run in parallel (capped,
  ~15–20 concurrent), and within each bundle the five generator agents
  (stance, sub-questions, assumptions, evidence, counterargument) also run in
  parallel — see [01](01-layers-and-standards.md).
...
Per request, as a function of `n`:

- **Generators:** `2` (context-gather) `+ 1` (frame) `+ 1` (breadth-scoping)
  `+ 5n` (perspective bundles) `+ 1` (global assumptions) `+ 1` (global
  evidence) `+ 1` (conclusions) `+ 1` (implications) `+ 1` (final
  composition) = **`5n + 9`**
- **Reviewers:** `9` (frame) `+ 9n` (perspectives) `+ 9` (global assumptions)
  `+ 9` (global evidence) `+ 9` (conclusions) `+ 9` (implications) =
  **`9n + 45`**
- **Total (happy path, no retries):** **`14n + 54`**
...
Retries can push the perspective-layer portion (`5n` generators + `9n`
reviewers = `14n`) up to roughly 3× in the worst case for whichever bundles
