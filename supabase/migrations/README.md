...
| `0039_house_layer_feedback.sql` | houses | `house_layer_feedback` (post-draft Q&A/correction thread, one row per turn) — `can_access_house`-scoped SELECT/INSERT RLS, same visibility as `house_activity`. ⚠ Shipped without the matching `authenticated` grant — see `0040` |
| `0040_house_layer_feedback_grant.sql` | houses | **Same fix as `0034`/`0035`/`0037`**: grant `select, insert` on `house_layer_feedback` to `authenticated` — `0039`'s RLS policies alone didn't grant base table access, and this table is written by the caller's own session (`app/api/houses/[id]/layer-feedback/route.ts`), not service role; confirmed live as `permission denied for table house_layer_feedback` |
