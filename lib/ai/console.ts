...
// reruns.md, migration 0043) — Loop C, sandbox reruns with a diff — adds a
// fourth DB-free contract, same rationale: FinalizeCandidateRequestSchema/
// CandidateSummary (the wire shape for app/api/houses/[id]/console/
// candidate/*), candidateBlocksNewSandbox (the "one live candidate at a
// time" rule, Trap 2's second half), candidateIsStale (Trap: staleness), and
// computeCandidateHouseState/diffCandidateStages (Trap 4: the diff is
// derived from the SAME reducer action a real rerun's completion already
// applies — lib/build/state.ts's APPLY_RERUN_RESULT case — never a second,
// parallel run-state-to-layer mapping). lib/build/state.ts's reducer is
// plain TS with no DOM/Supabase dependency, so importing it here doesn't
// break this file's "DB-free" posture — it's exactly the kind of pure logic
// this file already exists to hold.
